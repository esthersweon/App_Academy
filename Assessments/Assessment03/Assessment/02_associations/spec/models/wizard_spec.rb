...
describe Wizard do
  ...
  describe "Student" do
    ...
    describe "can take a course" do
      before(:each) do
        @asher = Wizard.create!(:fname => "Asher", :lname => "Abramson",
                                   :house_id => 2, :school_id => 1)

        @quidditch = Course.create!(:instructor_id => @asher.id, 
                                                :title => "Quidditch Practice",
                                                :description => "Be the flyest in the West")
        @quid_enrollment = CourseEnrollment.create!(:course_id => @quidditch.id, :student_id => harry.id)
      end
      

      it "and successfully enroll " do
        expect(harry.course_enrollments.last).to eq(@quid_enrollment)
      end

      it "and add the course to their list" do
        expect(harry.courses.last).to eq(@quidditch)
      end
