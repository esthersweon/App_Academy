...
def binary_search(array, target)
    ...
    current_index = array.length/2
    
    if target == array[current_index]
        current_index
        elsif target < array[current_index]
        binary_search(array.take(current_index), target)
        else
        new_index = binary_search(array.drop(current_index + 1), target)
        (new_index.nil?) ? nil : current_index + new_index + 1
    end
end
